    ...
    for group in attn_groups:
        ...
        for layer_name in group.layer_names:
            ...
            if layer_name in layer_tuple_strides:
                ...
                offset = raw_cache.storage_offset()
                is_index = isinstance(kv_cache_spec, AscendMLAAttentionSpec) and kv_cache_spec.scale_dim
                planes = [(kv_cache_spec.head_size, kv_cache_spec.dtype)]
                if is_index:
                    source_name = layer_name.removesuffix(".indexer.k_cache") + ".long_kv_cache"
                    offset += layer_kv_cache_spec[source_name].unpadded_page_size_bytes
                    planes.append((kv_cache_spec.scale_dim, kv_cache_spec.scale_dtype))
                views = []
                for head_size, dtype in planes:
                    ...
                    views.append(
                        torch.as_strided(
                            ...
                            size=shape,
                            stride=(layer_tuple_strides[layer_name] // dtype_size, *stride[1:]),
                            storage_offset=offset // dtype_size,
                        )
                    )
                    offset += stride[0] * dtype_size
