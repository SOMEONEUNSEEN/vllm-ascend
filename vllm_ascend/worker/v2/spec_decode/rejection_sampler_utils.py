...
from vllm_ascend.worker.v2.sample.gumbel import gumbel_block_argmax
...
# zero draws of the acceptance threshold u so tl.log(u) stays finite.
_TL_RAND_MIN = 4.6566127342e-10
...
def _rejection_kernel(
    ...
    # NPU: 0-d scalar accumulators via tl.zeros (verified pattern from the
    # MRV1 rejection kernels); int64 keeps accepted_length compatible with
    # the scalar tl.where update in the block verification path.
    ...
    for i in range(num_draft_tokens):
        ...
        if verifying:
            ...
            # tl_rand32; NPU Triton lacks scalar tl.rand, so generate u from
            # a 1-element block (same pattern as the Gumbel noise) and clamp
            # away from 0 so that tl.log(u) stays finite.
            ...
            if is_greedy:
                ...
                # NPU: use tl.where instead of the Python ternary IfExp,
                # which triton-ascend does not support with tensor
                # conditions.
                ...
                # NPU: no scalar masked load (upstream masks with
                # `i < num_draft_tokens - 1`), so clamp the next-token index
                # to stay in bounds and select -1 for the last step.
                next_idx = tl.minimum(logit_idx + 2, num_logits - 1)
                next_draft_token = tl.load(draft_sampled_ptr + next_idx).to(tl.int64)
                next_draft_token = tl.where(i < num_draft_tokens - 1, next_draft_token, -1)
                ...
                # NOTE: _compute_global_logprobs_and_logsumexp divides the
                # pre-temperature draft logits by `temp`, matching the stats
                # stored by _compute_local_logits_stats_kernel.
    ...
    if USE_BLOCK_VERIFICATION:
        if (is_greedy == 0) & (accepted_length < num_draft_tokens):
...
def _resample_kernel(
    ...
    # NPU: no scalar masked load (upstream masks with `not is_bonus`), so
    # clamp the next-token index to stay in bounds.
    next_token_idx = tl.minimum(resample_token_idx + 1, num_logits - 1)
    rejected_draft_token = tl.load(draft_sampled_ptr + next_token_idx)
    ...
    # NPU: use `|`/`== 0` instead of `or`/`not` with scalar tensors.
    use_target_logits = is_bonus | (is_valid_rejected_draft == 0)
    if use_target_logits:
        # Bonus token (no rejections) or -1 placeholder token. In either
        # case, directly use the target logits.
...
def rejection_sample(
    ...
    if use_fp64:
        # NPU Triton does not support float64 ops.
    ...
    if use_block_verification:
        ...
        # launched for full draft logits distributions. One-hot drafts use
        # a closed-form residual mass instead.
        if has_draft_logits:
