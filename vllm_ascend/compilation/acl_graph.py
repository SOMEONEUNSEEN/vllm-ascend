...
def update_full_graph_params(
    ...
    # get_impl_cls() (e.g. the DSA/SFA backends) and update_graph_params()
    # resolve backends via get_current_vllm_config(), which raises outside
    # set_current_vllm_config() on both 0.27.1 and main (e.g. draft query
    # graphs replayed from worker_busy_loop execute_dummy_batch), so always
    # run inside the config context.
